...
Type *get_expr_type(SemanticContext *ctx, ASTNode *expr) {
    static Type void_type = {TK_VOID, 0, NULL, NULL, NULL};
    ...
    case NODE_CALL: {
        ...
        if (!sym) {
            ...
            if (rk == TK_STRING || rk == TK_CLASS || rk == TK_ARRAY || rk == TK_HASH)
                expr->is_fresh_alloc = 1;
            return expr->resolved_type;
        }
        break;
...
/* Resolve a TypeInfo with object/tuple fields into a registered StructDef.
...
static void resolve_type_info(SemanticContext *ctx, TypeInfo *ti) {
    ...
    if (!sd) {
        ...
        sd->is_class = 1;
        unsigned int si = hash_struct_name(type_name);
        sd->next = ctx->struct_buckets[si];
        ctx->struct_buckets[si] = sd;
...
static void analyze_expr(SemanticContext *ctx, ASTNode *expr) {
        ...
        if (!sd) {
            ...
            for (NodeList *e = elems; e; e = e->next, idx++) {
                ...
                if (is_named) {
                    ...
                    if (tk == TK_ARRAY && rt && rt->elem)
                        fd->type->elem = type_clone(rt->elem);
                    if (tk == TK_HASH && rt) {
                        if (rt->elem) fd->type->elem = type_clone(rt->elem);
                        if (rt->key)  fd->type->key  = type_clone(rt->key);
                    }
                } else {
                    char fname[32];
                    snprintf(fname, sizeof(fname), "_%d", idx);
    ...
    case NODE_OBJECT_LITERAL: {
        ...
        for (NodeList *f = fields; f; f = f->next) {
            ...
            const char *suffix;
            if ((tk == TK_STRUCT || tk == TK_CLASS) && na->data.named_arg.value->resolved_type
                && na->data.named_arg.value->resolved_type->name) {
                suffix = na->data.named_arg.value->resolved_type->name;
            } else if (tk == TK_ARRAY) {
                suffix = "arr";
            } else if (tk == TK_HASH) {
                suffix = "hash";
            } else {
                suffix = type_kind_suffix(tk);
            }
            pos += snprintf(buf + pos, sizeof(buf) - pos, "_%s_%s",
                            na->data.named_arg.name, suffix);
        }
        ...
        if (!expr->resolved_type) expr->resolved_type = type_new(TK_CLASS);
        else expr->resolved_type->kind = TK_CLASS;
    ...
    case NODE_ARRAY_LITERAL: {
        ...
        for (NodeList *e = expr->data.array_literal.elems; e; e = e->next) {
            ...
            } else if (elem_type->kind != TK_UNKNOWN &&
                       e->node->resolved_type->kind != TK_UNKNOWN &&
                       !type_eq(elem_type, e->node->resolved_type)) {
    ...
    case NODE_HASH_LITERAL: {
        ...
        for (NodeList *p = expr->data.hash_literal.pairs; p; p = p->next) {
            ...
            } else if (key_type->kind != TK_UNKNOWN &&
                       pair->data.hash_pair.key->resolved_type->kind != TK_UNKNOWN &&
                       !type_eq(key_type, pair->data.hash_pair.key->resolved_type)) {
            ...
            } else if (val_type->kind != TK_UNKNOWN &&
                       pair->data.hash_pair.value->resolved_type->kind != TK_UNKNOWN &&
                       !type_eq(val_type, pair->data.hash_pair.value->resolved_type)) {
    ...
    case NODE_TYPED_EMPTY_ARRAY: {
        ...
        if (expr->data.typed_empty_array.elem_name) {
            ...
            TypeKind ek = (sd && sd->is_class) ? TK_CLASS : TK_STRUCT;
            expr->resolved_type->elem = type_new(ek);
    ...
    case NODE_TYPED_EMPTY_HASH: {
        ...
        if (expr->data.typed_empty_hash.value_name) {
            ...
            TypeKind vk = (sd && sd->is_class) ? TK_CLASS : TK_STRUCT;
            expr->resolved_type->elem = type_new(vk);
