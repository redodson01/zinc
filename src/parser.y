...
expr:
    ...
    | VAR IDENTIFIER ASSIGN expr        { $$ = make_var_decl($2, $4); $$->line = @1.first_line; }
    | LET IDENTIFIER ASSIGN expr        { $$ = make_let_decl($2, $4); $$->line = @1.first_line; }
    /* Assignments */
    | IDENTIFIER ASSIGN expr            { $$ = make_assign($1, $3); $$->line = @1.first_line; }
    | IDENTIFIER PLUS_ASSIGN expr       { $$ = make_compound_assign($1, "+=", $3); $$->line = @1.first_line; }
    | IDENTIFIER MINUS_ASSIGN expr      { $$ = make_compound_assign($1, "-=", $3); $$->line = @1.first_line; }
    | IDENTIFIER STAR_ASSIGN expr       { $$ = make_compound_assign($1, "*=", $3); $$->line = @1.first_line; }
    | IDENTIFIER SLASH_ASSIGN expr      { $$ = make_compound_assign($1, "/=", $3); $$->line = @1.first_line; }
    | IDENTIFIER PERCENT_ASSIGN expr    { $$ = make_compound_assign($1, "%=", $3); $$->line = @1.first_line; }
    /* Increment/decrement */
    | INCREMENT IDENTIFIER %prec PREFIX_INCDEC  { $$ = make_incdec($2, "++", 1); $$->line = @1.first_line; }
    | DECREMENT IDENTIFIER %prec PREFIX_INCDEC  { $$ = make_incdec($2, "--", 1); $$->line = @1.first_line; }
    | IDENTIFIER INCREMENT              { $$ = make_incdec($1, "++", 0); $$->line = @1.first_line; }
    | IDENTIFIER DECREMENT              { $$ = make_incdec($1, "--", 0); $$->line = @1.first_line; }
    /* Binary operators */
    | expr PLUS expr                    { $$ = make_binop($1, "+", $3); $$->line = @$.first_line; }
    | expr MINUS expr                   { $$ = make_binop($1, "-", $3); $$->line = @$.first_line; }
    | expr STAR expr                    { $$ = make_binop($1, "*", $3); $$->line = @$.first_line; }
    | expr SLASH expr                   { $$ = make_binop($1, "/", $3); $$->line = @$.first_line; }
    | expr PERCENT expr                 { $$ = make_binop($1, "%", $3); $$->line = @$.first_line; }
    | expr LT expr                      { $$ = make_binop($1, "<", $3); $$->line = @$.first_line; }
    | expr GT expr                      { $$ = make_binop($1, ">", $3); $$->line = @$.first_line; }
    | expr LE expr                      { $$ = make_binop($1, "<=", $3); $$->line = @$.first_line; }
    | expr GE expr                      { $$ = make_binop($1, ">=", $3); $$->line = @$.first_line; }
    | expr EQ expr                      { $$ = make_binop($1, "==", $3); $$->line = @$.first_line; }
    | expr NE expr                      { $$ = make_binop($1, "!=", $3); $$->line = @$.first_line; }
    | expr AND expr                     { $$ = make_binop($1, "&&", $3); $$->line = @$.first_line; }
    | expr OR expr                      { $$ = make_binop($1, "||", $3); $$->line = @$.first_line; }
    /* Unary operators */
...
unless_expr:
    UNLESS expr block                 { $$ = make_if(make_unaryop("!", $2), $3, NULL); $$->line = @1.first_line; }
    | UNLESS expr block ELSE block    { $$ = make_if(make_unaryop("!", $2), $3, $5); $$->line = @1.first_line; }
    ;
...
until_expr:
    UNTIL expr block      { $$ = make_while(make_unaryop("!", $2), $3); $$->line = @1.first_line; }
    ;
...
for_update:
    ...
    | IDENTIFIER SLASH_ASSIGN expr      { $$ = make_compound_assign($1, "/=", $3); $$->line = @1.first_line; }
    | IDENTIFIER PERCENT_ASSIGN expr    { $$ = make_compound_assign($1, "%=", $3); $$->line = @1.first_line; }
    | INCREMENT IDENTIFIER              { $$ = make_incdec($2, "++", 1); $$->line = @1.first_line; }
    | DECREMENT IDENTIFIER              { $$ = make_incdec($2, "--", 1); $$->line = @1.first_line; }
    | IDENTIFIER INCREMENT              { $$ = make_incdec($1, "++", 0); $$->line = @1.first_line; }
    | IDENTIFIER DECREMENT              { $$ = make_incdec($1, "--", 0); $$->line = @1.first_line; }
    ;
