...
ASTNode *make_binop(ASTNode *left, char *op, ASTNode *right) {
    ASTNode *n = alloc_node(NODE_BINOP);
    n->data.binop.left = left;
    n->data.binop.op = strdup(op);
...
ASTNode *make_compound_assign(char *name, char *op, ASTNode *value) {
    ASTNode *n = alloc_node(NODE_COMPOUND_ASSIGN);
    n->data.compound_assign.name = name;
    n->data.compound_assign.op = strdup(op);
    n->data.compound_assign.value = value;
    return n;
}

ASTNode *make_incdec(char *name, char *op, int is_prefix) {
    ASTNode *n = alloc_node(NODE_INCDEC);
    n->data.incdec.name = name;
    n->data.incdec.op = strdup(op);
...
ASTNode *make_let_decl(char *name, ASTNode *value) {
    ASTNode *n = alloc_node(NODE_LET_DECL);
    n->data.let_decl.name = name;
    n->data.let_decl.value = value;
    return n;
}
...
ASTNode *make_index_assign(ASTNode *object, ASTNode *index, ASTNode *value) {
    ASTNode *n = alloc_node(NODE_INDEX_ASSIGN);
    n->data.index_assign.object = object;
    n->data.index_assign.index = index;
...
ASTNode *make_field_assign(ASTNode *object, char *field, ASTNode *value) {
    ASTNode *n = alloc_node(NODE_FIELD_ASSIGN);
    n->data.field_assign.object = object;
    n->data.field_assign.field = field;
    n->data.field_assign.value = value;
    return n;
}
...
ASTNode *make_struct_def(char *name, NodeList *fields) {
    ASTNode *n = alloc_node(NODE_STRUCT_DEF);
    n->data.struct_def.name = name;
    n->data.struct_def.fields = fields;
    return n;
}

ASTNode *make_class_def(char *name, NodeList *fields) {
    ASTNode *n = alloc_node(NODE_CLASS_DEF);
    n->data.struct_def.name = name;
    n->data.struct_def.fields = fields;
    return n;
}
...
static void print_type_info(TypeInfo *ti) {
    ...
    case TK_STRUCT:
        if (ti->struct_name) printf("%s", ti->struct_name);
        else printf("struct");
        break;
...
void print_ast(ASTNode *node, int indent) {
    ...
    case NODE_COMPOUND_ASSIGN:
        ...
        print_ast(node->data.compound_assign.value, indent + 1);
        break;
    case NODE_INCDEC:
        printf("IncDec: %s%s%s\n",
               node->data.incdec.is_prefix ? node->data.incdec.op : "",
               node->data.incdec.name,
               node->data.incdec.is_prefix ? "" : node->data.incdec.op);
        break;
    case NODE_VAR_DECL:
    ...
    case NODE_LET_DECL:
        ...
        print_ast(node->data.let_decl.value, indent + 1);
        break;
    case NODE_IF:
        printf("If\n");
    ...
    case NODE_FIELD_ASSIGN:
        ...
        print_ast(node->data.field_assign.object, indent + 1);
        print_ast(node->data.field_assign.value, indent + 1);
        break;
    case NODE_INDEX:
        printf("Index\n");
    ...
    case NODE_STRUCT_DEF:
        ...
        for (NodeList *l = node->data.struct_def.fields; l; l = l->next)
            print_ast(l->node, indent + 1);
        break;
    case NODE_CLASS_DEF:
        printf("ClassDef: %s\n", node->data.struct_def.name);
    ...
    case NODE_STRUCT_INIT:
        printf("StructInit: %s\n", node->data.struct_init.name);
        for (NodeList *l = node->data.struct_init.args; l; l = l->next)
            print_ast(l->node, indent + 1);
        break;
    case NODE_NAMED_ARG:
    ...
    case NODE_INDEX_ASSIGN:
        printf("IndexAssign\n");
        print_ast(node->data.index_assign.object, indent + 1);
        print_ast(node->data.index_assign.index, indent + 1);
        print_ast(node->data.index_assign.value, indent + 1);
        break;
...
void free_ast(ASTNode *node) {
    ...
    case NODE_PARAM:
        ...
        free(node->data.param.type_info);
        break;
    case NODE_BINOP:
    ...
    case NODE_COMPOUND_ASSIGN:
        ...
        free(node->data.compound_assign.op);
        free_ast(node->data.compound_assign.value);
        break;
    case NODE_INCDEC:
        free(node->data.incdec.name);
        free(node->data.incdec.op);
        break;
    case NODE_VAR_DECL:
    ...
    case NODE_LET_DECL:
        ...
        free_ast(node->data.let_decl.value);
        break;
    case NODE_IF:
    ...
    case NODE_FIELD_ACCESS:
        ...
        break;
    case NODE_FIELD_ASSIGN:
        free_ast(node->data.field_assign.object);
        free(node->data.field_assign.field);
        free_ast(node->data.field_assign.value);
        break;
    case NODE_INDEX:
    ...
    case NODE_CLASS_DEF:
        ...
        free_list(node->data.struct_def.fields);
        break;
    case NODE_STRUCT_FIELD:
        free(node->data.struct_field.name);
        if (node->data.struct_field.type_info) {
            free(node->data.struct_field.type_info->struct_name);
            free(node->data.struct_field.type_info);
        }
        free_ast(node->data.struct_field.default_value);
        break;
    case NODE_STRUCT_INIT:
        free(node->data.struct_init.name);
        free_list(node->data.struct_init.args);
        break;
    case NODE_NAMED_ARG:
    ...
    case NODE_HASH_PAIR:
        ...
        break;
    case NODE_INDEX_ASSIGN:
        free_ast(node->data.index_assign.object);
        free_ast(node->data.index_assign.index);
        free_ast(node->data.index_assign.value);
        break;
    case NODE_EXTERN_BLOCK:
    ...
    case NODE_EXTERN_FUNC:
        ...
        free(node->data.extern_func.return_type);
        break;
    case NODE_EXTERN_VAR:
        free(node->data.extern_var.name);
        free(node->data.extern_var.type_info);
        break;
    case NODE_EXTERN_LET:
